...
namespace Moongy.RD.Launchpad.Generator.Contracts.Core.Validators
{
    public static class AccessValidator
    {
        ...
        {
            if (!isRequired && (access == null || access.Type == AccessEnum.NONE)) return;
            if (access == null || access.Type == AccessEnum.NONE) throw new ContractRequiresAccessException();
            if (access.Type == AccessEnum.OWNABLE) EthereumAddressValidator.Validate(access.Owner, true, "owner");
            if (access.Type == AccessEnum.MANAGED) 
            {
                if (access.Managers.Count() == 0) throw new ContractRequiresManagersException();
            ...
            {
                if (access.Roles.Count() == 0) throw new ContractRequiresRolesException();
