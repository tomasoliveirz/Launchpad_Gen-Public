...
namespace WebApi.Controllers
{
    ...
    public class ContractGenerationResultsController(IContractGenerationResultBusinessObject bo) : ControllerBase
    {
        ...
        {
            var result = await bo.CreateAsync(contractGenerationResult, contractGenerationResult.ContractVariant?.Uuid ?? Guid.Empty);
        ...
        [HttpPut("{uuid}")]
        public async Task<ActionResult> UpdateAsync(Guid uuid, ContractGenerationResult contractGenerationResult)
        {
            var result = await bo.UpdateAsync(uuid, contractGenerationResult, contractGenerationResult.ContractVariant?.Uuid);
            if (result.IsSuccessful) return Ok();
            if (result.Exception is NotFoundException nfe) return NotFound(nfe.Id);
            if (result.Exception is InvalidModelException ime) return BadRequest(ime.Message);
            return Problem(result.Exception?.Message ?? "");
